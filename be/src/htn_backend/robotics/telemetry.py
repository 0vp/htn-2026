...
class Control(BaseModel):
    """Who the firmware says holds control (robot/src/net/authority.cpp); not feedback."""

    model_config = ConfigDict(extra="forbid")
    owner: Literal["none", "human", "agent"]
    supervised: bool
    estop: bool
...
class Duty(BaseModel):
    model_config = ConfigDict(extra="forbid", allow_inf_nan=False)
    left: float = Field(ge=-1, le=1)
    right: float = Field(ge=-1, le=1)


class Encoders(BaseModel):
    """Raw quadrature counts; meaningless as distance until the encoders are calibrated."""
