...
/// Audio runs phone-to-OpenAI over WebRTC for GPT-Live's native latency; the server joins the
/// same session over a sideband socket for delegation and tool work.
...
struct VoiceAPI: VoiceServing {
    ...
    func create(sdp: String, codex: Bool, requestID: String) async throws -> VoiceSession {
        ...
        let body: [String: Any] = ["device_id": device, "codex_enabled": codex, "request_id": requestID, "sdp": sdp]
