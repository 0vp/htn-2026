...
type Listener = (batch: LidarBatch, origin: Origin) => void;

/**
 * Fan-out of point batches. Kept outside React so the renderer never waits on state.
 * During replay, live batches (socket, demo, console) are dropped so the two never mix.
 */
export class LidarFeed {
  ...
  emit(batch: LidarBatch, origin: Origin = 'live'): void {
    if (this.replaying && origin === 'live') return;
    for (const listener of this.listeners) listener(batch, origin);
  }

  setReplaying(replaying: boolean): void {
    this.replaying = replaying;
    for (const listener of this.listeners) listener({ xyz: new Float32Array(0), reset: true }, 'replay');
  }
}
