...
import { telemetryBus, type SampleOrigin } from './bus';
...
export function useTelemetry(): TelemetryFeed {
  ...
  useEffect(() => {
    const record = (sample: Telemetry, source: SampleOrigin) =>
      setFeed((f) => {
    ...
    const unsubscribe = telemetryBus.subscribe(record);
    const stopSource = startSource();
    return () => {
      unsubscribe();
      stopSource();
    };
...
/** Runs the simulator or the live socket, publishing 10 Hz samples to the bus. */
function startSource(): () => void {
  const record = (sample: Telemetry, origin: SampleOrigin) => telemetryBus.emit(sample, origin);
