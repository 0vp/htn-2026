import { autopilot } from '../control/goal';
...
export function createSim() {
  ...
  return function step(dt: number): Telemetry {
    const s = control.get();
    const duty = autopilot.active ? autopilot : mixDrive(s);
    ...
    const winchPos = t.winchPos.map((p, i) => {
      const v = canMove(s) ? s.winch[i] : 0;
      return Math.max(0, Math.min(1, p + v * 0.12 * dt));
    }) as Telemetry['winchPos'];
    const winchActive = !canMove(s) ? 0 : s.winch.filter((w, i) => w !== 0 && winchPos[i] > 0 && winchPos[i] < 1).length;
